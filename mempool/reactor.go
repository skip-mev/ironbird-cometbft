...
func (r *gossipRouter) adjustRedundancy(logger log.Logger) (float64, bool) {
	...
	if r.first >= r.config.TxsPerAdjustment {
		...
		targetRedundancyDelta := float64(r.config.TargetRedundancy) * float64(r.config.TargetRedundancyDelta) / 100.
		if redundancy < r.config.TargetRedundancy-targetRedundancyDelta {
			logger.Info("TX redundancy BELOW limit, increasing it",
				"redundancy", redundancy,
				"limit", r.config.TargetRedundancy+targetRedundancyDelta,
			)
			sendReset = true
		} else if r.config.TargetRedundancy+targetRedundancyDelta <= redundancy {

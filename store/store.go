...
func NewBlockStore(db storage.DB, options ...BlockStoreOption) *BlockStore {
	start := time.Now()
...
func (bs *BlockStore) PruneBlocks(height int64, state sm.State) (uint64, int64, error) {
	...
	defer batch.Close()
	flush := func(batch storage.Batch, base int64) error {
		// We can't trust batches to be atomic, so update base first to make sure no
		// one tries to access missing blocks.
		bs.mtx.Lock()
		defer batch.Close()
		defer bs.mtx.Unlock()
...
func (bs *BlockStore) saveBlockPart(
	height int64, index int,
	part *types.Part,
	batch storage.Batch,
	saveBlockPartsToBatch bool,
) {
...
func (bs *BlockStore) saveStateAndWriteDB(batch storage.Batch, errMsg string) error {
...
func SaveBlockStoreState(bsj *cmtstore.BlockStoreState, batch storage.Batch) {
...
func LoadBlockStoreState(db storage.DB) cmtstore.BlockStoreState {

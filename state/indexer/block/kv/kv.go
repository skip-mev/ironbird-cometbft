...
func (idx *BlockerIndexer) matchRange(
	...
	it, err := storage.PrefixIterator(idx.store, startKey)
	if err != nil {
...
func (*BlockerIndexer) setTmpHeights(
...
func (idx *BlockerIndexer) match(
	...
	case c.Op == syntax.TEq:
		it, err := storage.PrefixIterator(idx.store, startKeyBz)
		if err != nil {
			return nil, fmt.Errorf("failed to create prefix iterator: %w", err)
		}
		defer it.Close()
	...
	case c.Op == syntax.TExists:
		...
		it, err := storage.PrefixIterator(idx.store, prefix)
		if err != nil {
			return nil, fmt.Errorf("failed to create prefix iterator: %w", err)
		}
		defer it.Close()
...
func (idx *BlockerIndexer) indexEvents(
	batch storage.Batch,
	events []abci.Event,
	height int64,
) error {
